...
계산 엔진(`calculator.ts`)의 로직 무결성을 검증하는 23개 테스트 케이스를 작성했습니다.
...
| 시나리오 | 설명 |
|---|---|
| 문턱 미달 | 총급여 25% 미달 시 공제액 0 및 잔여 금액 반환 검증 |
| 경계값 처리 | 정확히 25% 달성 시 공제 미시작, 초과 즉시 공제 시작 |
| 공제율 정확도 | 신용카드 15% · 체크카드 30% · 전통시장 40% 및 혼용 시 우선순위 |
| 한도 캡핑 | 공제액이 한도 초과 시 상한선 적용 검증 |
| 독립 한도 적용 | 전통시장·대중교통 각각 독립 100만 원 한도 (합산 오류 방지) |
| 자녀 수 기반 한도 | 자녀 0/1/2명 및 3명 이상 클램핑 처리 검증 |
| 소득 구간별 예외 | 7천만 초과 시 culture/sports 자동 제외 및 한도 테이블 전환 |
